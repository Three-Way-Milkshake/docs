...
In un \gls{progetto}\textsubscript{G} di queste dimensioni è possibile incontrare problemi di varia natura. Per arginare i rischi si possono gestire attentamente 4 \gls{attivita}\textsubscript{G}:

\begin{itemize}
	\item \textbf{Individuazione dei rischi}: individuare i fattori che possono introdurre criticità nello svolgimento del \gls{progetto}\textsubscript{G};
	\item \textbf{Analisi dei rischi}: esaminare i fattori di rischio stimando la probabilità che la criticità si manifesti, l'impatto che ha e le sue conseguenze nel \gls{progetto}\textsubscript{G};
...
\begin{longtable} { 
	...
	\textbf{Nome}: \\ Difficoltà nella compresione dei \glspl{requisito}\textsubscript{G}
	& \textbf{Codice}: \\ RIS\_T - 3  
	& \textbf{Occorrenza}: Alta \\ \textbf{Pericolosità}: Bassa
	
	\tabularnewline
	
	\textbf{Descrizione}: \\ Il documento di descrizione del \gls{capitolato}\textsubscript{G} fornisce una definizione generale del problema ma risulta poco dettagliato in termini di \glspl{requisito}\textsubscript{G} da soddisfare. Inoltre il gruppo può trovare difficoltà nella comprensione dei \glspl{requisito}\textsubscript{G} di un prodotto di cui non si figura ancora l'implementazione.
	& 
	\textbf{Rilevamento}: \\ Il gruppo non riesce a definire con precisione quali \glspl{requisito}\textsubscript{G} siano richiesti dal prodotto e ha difficoltà a definire con precisione il comportamento degli \glspl{attore}\textsubscript{G} coinvolti.
	
	&  
	\textbf{Piano di contingenza}: \\ Verrà fissato un incontro sincrono con il proponente al fine di definire i \glspl{requisito}\textsubscript{G} del prodotto. La comunicazione continuerà in maniera asincrona per trovare conferma delle assunzioni fatte durante l'Analisi dei Requisiti.
...
\begin{longtable} { 
	...
	\textbf{Descrizione}: \\ Preventivare le ore necessarie a svolgere le \gls{attivita}\textsubscript{G} future è difficile se non si ha maturato esperienza nello sviluppo di \glspl{progetto}\textsubscript{G} complessi. \\
	...
	&  
	\gls{cruscotto}\textsubscript{G} di \gls{progetto}\textsubscript{G}. Significativi discostamenti tra preventivo e costi effettivamente sostenuti verranno comunicati con tempestività al proponente.
...
\begin{longtable} { 
	...
	& 
	\textbf{Piano di contingenza}: \\ Ogni membro del gruppo è tenuto a tenere un atteggiamento aperto al dialogo e al compromesso, conscio del fatto che la buona riuscita del \gls{progetto}\textsubscript{G} è imprescindibile da una stretta collaborazione interna. Dopo ogni \gls{revisione}\textsubscript{G} si terrà un'attività di verifica in cui ogni membro avrà l'opportunità di esporre 
	&  
