...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item la conferma dell’inserimento della nuova \gls{task}\textsubscript{G} (UC4.3);
		\item l’eliminazione di una \gls{task}\textsubscript{G} dalla lista (UC4.4);
		\item la modifica della priorità di una \gls{task}\textsubscript{G} esistente (UC4.5);
	\end{itemize}
...
\begin{itemize}
	...
	\item 	\textbf{Descrizione:} il responsabile inserisce nella lista dei \acrshort{POI}\textsubscript{A} da soddisfare dagli operatore un nuovo POI;
...
\begin{itemize}
	...
	\item 	\textbf{Scenario principale:} il responsabile seleziona dalla lista delIe \gls{task}\textsubscript{G} quella che intende eliminare e procede alla cancellazione premendo l’apposito pulsante;
